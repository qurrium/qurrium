...
from .rho_mk_cell import rho_m_cell, rho_m_cell_precomputed, rho_m_cell_of_flatten
...
RhoMCoreMethod = Union[Literal["numpy", "numpy_precomputed", "numpy_flatten"], str]
...
def rho_m_core_py(
    ...
    if rho_method == "numpy_flatten":
        ...
        rho_m_list = [
            rho_m_cell_of_flatten(bits_array, count_num)
            for bits_array, count_num in flatten_recount_list_vectorized
        ]

    else:
        cell_calculation_method = (
            rho_m_cell_precomputed if rho_method == "numpy_precomputed" else rho_m_cell
        )
        rho_m_list = [
            cell_calculation_method(single_counts, random_unitary_um[idx], selected_clregs_sorted)
...
def rho_m_core(
    ...
    if rho_method not in ["numpy", "numpy_precomputed", "numpy_flatten"]:
        raise ValueError(
